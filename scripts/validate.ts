...
function checkI18nHardcoded(
  ...
  if (!isTsxFile(filePath) || isExcluded(filePath) || isStoryFile(filePath)) return { violations: [], passed: true };
  const violations: Violation[] = [];
  // Detect: >Some English Text< (capitalized words in JSX content)
...
function checkImportOrder(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let group: number;
    if (/from\s+['"]react['"]/.test(line) || /from\s+['"]react[-/]/.test(line)) {
      group = 1; // React
    } else if (/from\s+['"]@moloco\//.test(line)) {
      group = 3; // Moloco UI
    } else if (/from\s+['"]@msm-portal\//.test(line)) {
      group = 4; // Internal
    ...
    importLines.push({ line: i + 1, text: line, group });
  }
